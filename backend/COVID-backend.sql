...
CREATE TABLE IF NOT EXISTS account_admins (
	...
	totp_seed VARCHAR(32) NOT NULL, -- account_admin specific seed for HMAC-TOTP authentication.
	email VARCHAR(255) NOT NULL, -- E-mail address of healthcare_worker.
	phone_number VARCHAR(20) NOT NULL, -- Phone number on which the account_admin has to be near-instantly available so we can reach them if their account shows suspicious activity.
...
CREATE TABLE IF NOT EXISTS account_admin_sessions (
	...
	KEY (session_token, account_admin_uuid),
...
CREATE TABLE IF NOT EXISTS healthcare_workers (
	...
	totp_seed VARCHAR(32) NOT NULL, -- healthcare_worker specific seed for HMAC-TOTP authentication.
	email VARCHAR(255) NOT NULL, -- E-mail address of healthcare_worker.
	phone_number VARCHAR(20) NOT NULL, -- Phone number on which the healthcare_worker has to be near-instantly available so we can reach them if their account shows suspicious activity.
	reset_code VARCHAR(64) NULL DEFAULT NULL, -- Password reset code.
	active BOOLEAN NOT NULL DEFAULT FALSE, -- Is account active?
	account_expiration_date DATE NOT NULL DEFAULT DATE_ADD(CURDATE(), INTERVAL 2 YEAR), -- Automatic expiration after 2 years.
...
CREATE TABLE IF NOT EXISTS account_admin_actions (
	...
	healthcare_worker_uuid VARCHAR(36) NOT NULL, -- Unique UUID of healthcare_worker the action is performed on.
	FOREIGN KEY (account_admin_uuid) REFERENCES account_admins(account_admin_uuid) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS daily_tracing_key_activation_requests (
	...
	healthcare_worker_uuid VARCHAR(36) NULL, -- UUID of healthcare_worker who made the activation request.
	valid_until TIMESTAMP NOT NULL DEFAULT DATE_ADD(NOW(), INTERVAL 2 HOUR), -- Expiration timestamp of the healthcare_worker's session.
